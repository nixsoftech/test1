...
# Reboots after dist-upgrade if required, reboots after each upgrade, and confirms 22.04 upgrade
# Resumes from failure point with idempotent states and handles reboots with background execution
# Note: After each reboot, the script may terminate with "minion did not return . not connected".
# Re-run the script (salt '10.13.0.81' state.apply os_upgrade_gk1) after the minion is back online to resume.

# Check if a reboot is required
check_reboot_required:
  cmd.run:
    - name: '[ -f /var/run/reboot-required ] && echo "Reboot required, will initiate reboot" || echo "No reboot required"'
    - shell: /bin/bash

# Reboot if required at the start
reboot_if_required_initial:
  cmd.run:
    - name: 'nohup reboot &'
    - shell: /bin/bash
    - onlyif: '[ -f /var/run/reboot-required ]'
    - unless: 'test -f /var/run/salt_upgrade_reboot_initial || lsb_release -sr | grep -q "20.04\|22.04"'
    - require:
      - cmd: check_reboot_required
    - onchanges:
      - cmd: check_reboot_required
    - creates: '/var/run/salt_upgrade_reboot_initial'
...
remove_lxd:
  pkg.removed:
    ...
    - require:
      - cmd: reboot_if_required_initial
...
update_sources_list_20_04:
  file.managed:
    ...
    - unless: 'grep -q "focal" /etc/apt/sources.list'
...
update_apt_cache_20_04:
  cmd.run:
    ...
    - unless: 'lsb_release -sr | grep -q "20.04"'
...
reboot_after_dist_upgrade_20_04:
  cmd.run:
    ...
    - unless: 'test -f /var/run/salt_upgrade_reboot_dist_20_04 || lsb_release -sr | grep -q "20.04"'
    - creates: '/var/run/salt_upgrade_reboot_dist_20_04'
...
reboot_after_20_04:
  cmd.run:
    ...
    - unless: 'test -f /var/run/salt_upgrade_reboot_20_04 || lsb_release -sr | grep -q "20.04"'
    - creates: '/var/run/salt_upgrade_reboot_20_04'
...
update_sources_list_22_04:
  file.managed:
    ...
    - unless: 'grep -q "jammy" /etc/apt/sources.list'
...
update_apt_cache_22_04:
  cmd.run:
    ...
    - unless: 'lsb_release -sr | grep -q "22.04"'
...
reboot_after_dist_upgrade_22_04:
  cmd.run:
    ...
    - unless: 'test -f /var/run/salt_upgrade_reboot_dist_22_04 || lsb_release -sr | grep -q "22.04"'
    - creates: '/var/run/salt_upgrade_reboot_dist_22_04'
...
reboot_after_22_04:
  cmd.run:
    ...
    - unless: 'test -f /var/run/salt_upgrade_reboot_22_04 || lsb_release -sr | grep -q "22.04"'
    - creates: '/var/run/salt_upgrade_reboot_22_04'
