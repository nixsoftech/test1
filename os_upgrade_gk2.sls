...
# Resumes from failure point with idempotent states and handles initial reboot requirement

# Check if a reboot is required and reboot if needed
check_reboot_required:
  cmd.run:
    - name: '[ -f /var/run/reboot-required ] && echo "Reboot required, initiating reboot" || echo "No reboot required"'
    - shell: /bin/bash
...
# Wait for Salt minion to come back online after initial reboot
wait_for_reboot_initial:
  cmd.run:
    - name: |
        for i in {1..60}; do
